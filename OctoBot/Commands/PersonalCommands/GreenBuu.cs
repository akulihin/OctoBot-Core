    ...
    internal static class GreenBuuTimerClass
    {
        ...
        private static async void GreenBuuTimer(object sender, ElapsedEventArgs e)
        {
            ...
            if (Event == 228)
            {
                ...
                //Random User
                var rng = new Random();
                var randomSocketGuildUser = Global.Client.GetGuild(375104801018609665).Users.OrderBy(r => rng.Next()).FirstOrDefault();
                ///////////////
              //  var newColor = await Global.Client.GetGuild(375104801018609665).CreateRoleAsync(name: "RED", permissions: null, color: Color.Red, isHoisted: true);
                ...
                var mutedRole = Global.Client.GetGuild(375104801018609665).Roles.SingleOrDefault(x => x.Name.ToString() == "Muted");
