   ...
   public class UpdateShadowMess : ModuleBase<SocketCommandContext>
    {
        ...
        public async Task Boolek()
        {
          ...
          //  await Context.Message.DeleteAsync();
            ...
            embed.AddInlineField("Роль для рум", "Чтобы **получить** или **снять** роль просто нажмите определенную эмоцию, или пропишите соответствующую команду в <#374914059679694848>\n" +
                                                 "**________**\n");
            ...
            embed.AddInlineField($"Игровые Комнаты:", $"!hots - <#421637740137021450> и <#425354467022602258> Нет времени объяснять ─ фокуси танка {new Emoji("<:hanzo:445324859690582018>")}\n" +
        ...
        public async Task RoomApd()
        {
            ...
            {/*
                ...
                */
                ...
                embed.AddInlineField("Роль для рум",
                    ...
                    "**________**\n");
                embed.AddInlineField("Блокирующие каналы роли:",
                ...
                embed.AddInlineField($"Добавляющие каналы роли:",
                    $"{new Emoji("<:WoahMorfin:436787514813186050>")} - !voice - <#421951522700787713> когда-нибудь тут проведут мафию, свояк, или еще что-нибудь\n" +
                    ...
                    $"{new Emoji("<:PogChamp:374656108117098517>")} - !cards - ты тоже любишь поиграть ручками с... рандомом? <#438955526450315265> <#438955581965860864>\n" +
                    $"**________**\n");

                embed.AddInlineField($"Игровые Комнаты:",
                    $"{new Emoji("<:hanzo:445324859690582018>")} - !hots - <#421637740137021450> и <#425354467022602258> Нет времени объяснять ─ фокуси танка\n" +
                    $"{new Emoji("<:Minecrafticonfilegzpvzfll:445325226427940874>")} - !minecraft - <#425654423642177536> это то место, где ты сможешь построить **САМУЮ КРАСИВУЮ КОРОБКУ**, лучше из грязи\n" +
                    ...
                    $"**________**\n");
                    embed.AddInlineField("Нове румы:",  $"{new Emoji("<:warframe:445467639242948618>")} - !warframe - симулятор фермера в космосе\n" +
                ...
                var message = await Global.Client.GetGuild(338355570669256705)
                    .GetTextChannel(374627268162617344)
                    .GetMessageAsync(445502492088860672) as IUserMessage;
        ...
        public async Task ColorApd()
        {
            ...
            embed.WithColor(Color.Green);
                  embed.AddField("Роль для цвета",
                      $"Чтобы **получить** или **снять** роль нажмите определенную эмоцию, или пропишите соответствующую команду в <#374914059679694848>\n" +
                      $"**_______**\n");
                  embed.AddInlineField("Тыки на эмоцию", $"{new Emoji("<:rem:445275743719522304>")} <@&374900834946908160> - последователи анимэ (!weeb)\n{new Emoji("🦊")} <@&375079829642412034>- могущественные лисы и лисицы! (!fox)\n" +
                $"{new Emoji("<:PeaceKeepo:438257037667729408>")} <@&374900824880447489> - Авэ Мария! (!deus)\n" +
                $"{new Emoji("<:Steampunk:445276776676196353>")} <@&374900827632041986> - нет того, чего нельзя изобрести на паровом движке (!steampunk)\n" +
                $"{new Emoji("<:praise:445274481917952001>")} <@&440420047005941761> - Praise The Sun (!praise)\n{new Emoji("<:monkaS:398183436613058570>")} <@&374900838096961546> - последователи великого Пепе (!meme)\n" +
                $"{new Emoji("🐲")} <@&374900821382529025> - лазурные драконы! (!dragon)\n{new Emoji("🐼")} <@&374980985281970206> - злобные  существа - Панды  (!panda)\n" +
            ...
            var message = await Global.Client.GetGuild(338355570669256705)
                .GetTextChannel(374627268162617344)
                .GetMessageAsync(445501974608216064) as IUserMessage;
